"""
Delete duplicate conversations from FreeScout.

Usage:
    python delete_duplicate_conversations.py --confirm
"""
import json
import sys
import argparse
...
def main():
    parser = argparse.ArgumentParser(description='Delete duplicate conversations from FreeScout')
    parser.add_argument('--confirm', action='store_true', help='Confirm deletion (required)')
    args = parser.parse_args()

    # Load the list of duplicates to delete
    with open('duplicate_conversations_to_delete.json', 'r') as f:
        data = json.load(f)
    ...
    if not args.confirm:
        ...
        print(f'Command: python delete_duplicate_conversations.py --confirm')
        return 1
